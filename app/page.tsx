...
export default function StudentApp() {
  ...
  const [showLateNightWarning, setShowLateNightWarning] = useState(false); // ★ 深夜帯警告用
  ...
  return (
    <div className="min-h-screen bg-slate-50 text-slate-800 pb-10">
      ...
      {/* ★ 新機能：深夜帯（午前0時〜4時）警告ポップアップ */}
      {showLateNightWarning && (
        <div className="fixed inset-0 bg-black/70 flex items-center justify-center p-4 z-50">
          <div className="bg-white rounded-2xl p-6 w-full max-w-sm shadow-2xl text-center space-y-4 smooth-modal border-2 border-amber-400">
            <div className="text-4xl">🌙</div>
            <h3 className="text-base font-bold text-slate-800">夜遅くの勉強おつかれさま！</h3>
            <div className="p-3 bg-amber-50 rounded-xl border border-amber-200 text-xs text-amber-900 leading-relaxed font-medium">
              夜遅くまでの学習は体に毒です。明日も元気に過ごすために、今日は早めに寝ましょうね！💤
            </div>
            <button
      ...
      {/* 更新情報通知ポップアップ */}
      {showUpdateNoticePopup && !needsNameRegistration && (
            ...
            <div className="bg-white p-6 rounded-2xl border border-slate-100 shadow-sm space-y-3">
              ...
              <ul className="space-y-3 text-[11px] text-slate-600">
                <li className="border-b border-slate-100 pb-2">
                  ...
                  <p className="font-bold text-amber-600 mb-0.5">【お知らせ】深夜帯の利用注意ポップアップ追加</p>
                  <p className="font-medium text-slate-700">午前0時〜4時の間にアプリにアクセスした際、早めの就寝を促すアナウンスが表示されるようになりました。</p>
                </li>
                <li className="border-b border-slate-100 pb-2">
