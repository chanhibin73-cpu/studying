...
export default function TutorApp() {
  ...
  const loadAllData = async () => {
    ...
    try {
      const [usersSnap, qSnap, answersSnap, qToTutorSnap] = await Promise.all([
        getDocs(collection(db, 'users')),
        getDocs(collection(db, 'questions')),
        getDocs(collection(db, 'userAnswers')),
        getDocs(collection(db, 'questionsToTutor')),
      ]);
      ...
      <header className="bg-slate-900 text-white px-4 py-3 flex items-center justify-between sticky top-0 z-30 shadow-md">
        <div className="flex items-center gap-2">
          <button
            ...
            className="flex items-center gap-1 bg-slate-800 hover:bg-slate-700 active:scale-95 text-xs font-bold px-2.5 py-1.5 rounded-lg border border-slate-700 transition disabled:opacity-50"
            title="最新データに更新"
          >
          ...
          <div className="flex items-center gap-1.5 ml-1">
            ...
            <h1 className="font-bold text-base">チューター管理</h1>
          </div>
        </div>
